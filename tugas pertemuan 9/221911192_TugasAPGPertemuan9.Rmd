...
(b) Given your solution in (a), obtain the rotated loadings form = 2 and m = 3. Compare the two sets of rotated loadings. Interpret the m = 2 and m = 3 
factor solutions. 
...
```{r}
library(factoextra)
factorPCA <- summary(princomp(salesPeople, cor = FALSE), loadings = TRUE)
factorPCA
```
```{r}
...
(c)
```{r}
SSalesPeople <- cov(salesPeople)
eigenSalespeople <- eigen(cov(salesPeople))
eigenSalespeopleValue <- eigenSalespeople$values
eigenSalespeopleVector <- eigenSalespeople$vectors
LPC <- matrix(c(0), ncol = ncol(salesPeople), nrow = ncol(salesPeople))
for(i in 1:ncol(salesPeople))
{
        LPC[,i] <- sqrt(eigenSalespeopleValue[i])*eigenSalespeopleVector[,i]
}

specificVariance <- cov(salesPeople)-LsalesPeople
for(i in 1:ncol(salesPeople))
{
        for(j in 1:ncol(salesPeople))
        {
                if(i == j)
                {
                        specificVariance[i,j] <- specificVariance[i,j]
                } else{
                        specificVariance[i,j] <- 0
                }
        }
}
Communalities <- matrix(c(0), nrow = 1, ncol = ncol(salesPeople))
for(i in 1:ncol(salesPeople))
{
        for(j in 1:ncol(salesPeople))
        {
                Communalities[,i] <- Communalities[,i] + (LsalesPeople[i,j])^2
        }
}

Communalities
specificVariance
LPC%*%t(LPC)+specificVariance
```

## 9.20
Using the air-pollution variables X1, X2, X5, and X6 given in Table 1.5, generate the sample covariance matrix. 

(a) Obtain the principal component solution to a factor model with m = 1 andm = 2. 
